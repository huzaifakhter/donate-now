...
import { Play, Pause, Edit, Trash2, Megaphone, Plus, Share2, Check } from "lucide-react";
...
export default function FundraiserCampaignList({
  initialCampaigns,
}: {
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <Link
          ...
          className="h-9 px-4 py-2 bg-primary text-white hover:bg-primary/90 text-sm font-medium rounded-md shadow transition-colors flex items-center justify-center gap-2 cursor-pointer w-fit shrink-0 animate-in fade-in duration-100"
        >
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-200/40 text-xs text-on-surface">
              {campaigns.length === 0 ? (
                <tr>
                  <td colSpan={5} className="px-5 py-8 text-center text-on-surface-variant font-medium animate-pulse">
                    No campaigns found. Launch one above!
                  </td>
                </tr>
              ) : (
                campaigns.map((c) => {
                  ...
                  return (
                    <tr key={c.id} className="hover:bg-slate-50/30">
                      <td className="px-5 py-3 max-w-[240px]">
                        ...
                        <div className="text-[10px] text-on-surface-variant font-medium line-clamp-1 mt-0.5">
                          {c.description}
                        </div>
                      </td>
                      ...
                      <td className="px-5 py-3 text-right">
                        <div className="flex items-center justify-end gap-1.5">
                          ...
                          <button
                            ...
                            className="h-8 w-8 rounded-md border border-slate-200/60 bg-background hover:bg-slate-50 hover:text-slate-900 text-slate-500 flex items-center justify-center transition-colors cursor-pointer"
                          ...
                          <Link
                            href={`/fundraiser/campaigns/${c.id}/edit`}
                            className="h-8 w-8 rounded-md border border-slate-200/60 bg-background hover:bg-slate-50 hover:text-slate-900 text-slate-500 flex items-center justify-center transition-colors cursor-pointer"
                            title="Edit details"
                          >
